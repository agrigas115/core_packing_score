function [] = generate_surface_10(PDB_name)
PDB_name1 = strcat(PDB_name,'.txt');
position_file_name = strcat(PDB_name,'_total_position_10.txt');
f_write = fopen(position_file_name,'w');

f2 = fopen(strcat(PDB_name,'_param_10.lua'),'w');
fprintf(f2,'positionfile = ''%s''\n',position_file_name);
fprintf(f2,'readfile = ''./read.lua''\n\n');
...
xmin = 1*min(XtalPosition(:,1))-10;
xmax = 1*max(XtalPosition(:,1))+10;
ymin = 1*min(XtalPosition(:,2))-10;
ymax = 1*max(XtalPosition(:,2))+10;
zmin = 1*min(XtalPosition(:,3))-10;
zmax = 1*max(XtalPosition(:,3))+10;
